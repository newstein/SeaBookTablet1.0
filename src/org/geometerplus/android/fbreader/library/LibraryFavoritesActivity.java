...
import com.sean.android.seabooktablet.R;
...
public class LibraryFavoritesActivity extends LibraryBaseActivity {
    private LinkedList<FBTree> myItems;
    private TopLevelTreefavorites mySearchResultsItem;
...
//sean_0517     requestWindowFeature(Window.FEATURE_NO_TITLE);
        ...
        myItems.add(new TopLevelTreefavorites(
            ...
            new OpenTreeRunnable(LibraryInstance, new StartTreeActivityRunnable(PATH_FAVORITES, null) {
                public void run() {
                    if (LibraryInstance.favorites().hasChildren()) {
                        ...
                        UIUtil.showErrorMessage(LibraryFavoritesActivity.this, "noFavorites");
                    }
                }
            })
        ));
/*        myItems.add(new TopLevelTree(
        ...
        myItems.add(new TopLevelTree(
            ...
            new Runnable() {
                public void run() {
                    startActivity(
                        new Intent(LibraryFavoritesActivity.this, FileManager.class)
       ...
       // this.finish();
        ...
        TopLevelTreefavorites tree = (TopLevelTreefavorites)((LibraryAdapter)getListAdapter()).getItem(0);
        tree.run();           
    ...
    public void onListItemClick(ListView listView, View view, int position, long rowId) {
        TopLevelTreefavorites tree = (TopLevelTreefavorites)((LibraryAdapter)getListAdapter()).getItem(position);
        tree.run();
    }
    ...
    private void setSearchResults(Intent intent) {
        ...
        mySearchResultsItem = new TopLevelTreefavorites(
...
class TopLevelTreefavorites extends FBTree implements ZLAndroidTree {
    private final ZLResource myResource;
    private final String myParameter;
    private final int myCoverResourceId;
    private final Runnable myAction;

    public TopLevelTreefavorites(ZLResource resource, String parameter, int coverResourceId, Runnable action) {
        myResource = resource;
    ...
    public TopLevelTreefavorites(ZLResource resource, int coverResourceId, Runnable action) {
