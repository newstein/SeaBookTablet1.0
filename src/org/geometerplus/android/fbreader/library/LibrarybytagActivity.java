...
public class LibrarybytagActivity extends LibraryBaseActivity {
    private LinkedList<FBTree> myItems;
    private TopLevelTreebytag mySearchResultsItem;
...
/*        myItems.add(new TopLevelTree(
            ...
            new OpenTreeRunnable(LibraryInstance, new StartTreeActivityRunnable(PATH_FAVORITES, null) {
                public void run() {
                    if (LibraryInstance.favorites().hasChildren()) {
                        ...
                        UIUtil.showErrorMessage(LibrarybytagActivity.this, "noFavorites");
                    }
                }
            })
        ));
        ...
        myItems.add(new TopLevelTreebytag(
...
/*        myItems.add(new TopLevelTree(
            ...
            new Runnable() {
                public void run() {
                    startActivity(
                        new Intent(LibrarybytagActivity.this, FileManager.class)
        ...
        TopLevelTreebytag tree = (TopLevelTreebytag)((LibraryAdapter)getListAdapter()).getItem(0);
        tree.run();          
    }
    ...
    public void onListItemClick(ListView listView, View view, int position, long rowId) {
        TopLevelTreebytag tree = (TopLevelTreebytag)((LibraryAdapter)getListAdapter()).getItem(position);
        tree.run();
    }
    ...
    private void setSearchResults(Intent intent) {
        ...
        mySearchResultsItem = new TopLevelTreebytag(
...
class TopLevelTreebytag extends FBTree implements ZLAndroidTree {
    private final ZLResource myResource;
    private final String myParameter;
    private final int myCoverResourceId;
    private final Runnable myAction;

    public TopLevelTreebytag(ZLResource resource, String parameter, int coverResourceId, Runnable action) {
        myResource = resource;
    ...
    public TopLevelTreebytag(ZLResource resource, int coverResourceId, Runnable action) {
