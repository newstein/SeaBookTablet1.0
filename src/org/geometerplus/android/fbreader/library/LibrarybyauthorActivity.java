...
public class LibrarybyauthorActivity extends LibraryBaseActivity {
    private LinkedList<FBTree> myItems;
    private TopLevelTreebyauthor mySearchResultsItem;
...
/*        myItems.add(new TopLevelTree(
            ...
            new OpenTreeRunnable(LibraryInstance, new StartTreeActivityRunnable(PATH_FAVORITES, null) {
                public void run() {
                    if (LibraryInstance.favorites().hasChildren()) {
                        ...
                        UIUtil.showErrorMessage(LibrarybyauthorActivity.this, "noFavorites");
                    }
                }
            })
        ));
        ...
        myItems.add(new TopLevelTreebyauthor(
...
/*        myItems.add(new TopLevelTree(
        ...
        myItems.add(new TopLevelTree(
            ...
            new Runnable() {
                public void run() {
                    startActivity(
                        new Intent(LibrarybyauthorActivity.this, FileManager.class)
        ...
        TopLevelTreebyauthor tree = (TopLevelTreebyauthor)((LibraryAdapter)getListAdapter()).getItem(0);
        tree.run();         
    ...
    public void onListItemClick(ListView listView, View view, int position, long rowId) {
        TopLevelTreebyauthor tree = (TopLevelTreebyauthor)((LibraryAdapter)getListAdapter()).getItem(position);
        tree.run();
    }
    ...
    private void setSearchResults(Intent intent) {
        ...
        mySearchResultsItem = new TopLevelTreebyauthor(
...
class TopLevelTreebyauthor extends FBTree implements ZLAndroidTree {
    private final ZLResource myResource;
    private final String myParameter;
    private final int myCoverResourceId;
    private final Runnable myAction;

    public TopLevelTreebyauthor(ZLResource resource, String parameter, int coverResourceId, Runnable action) {
        myResource = resource;
    ...
    public TopLevelTreebyauthor(ZLResource resource, int coverResourceId, Runnable action) {
