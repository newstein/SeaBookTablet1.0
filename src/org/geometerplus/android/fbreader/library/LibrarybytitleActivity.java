...
public class LibrarybytitleActivity extends LibraryBaseActivity {
    private LinkedList<FBTree> myItems;
    private TopLevelTreebytitle mySearchResultsItem;
...
/*        myItems.add(new TopLevelTree(
            ...
            new OpenTreeRunnable(LibraryInstance, new StartTreeActivityRunnable(PATH_FAVORITES, null) {
                public void run() {
                    if (LibraryInstance.favorites().hasChildren()) {
                        ...
                        UIUtil.showErrorMessage(LibrarybytitleActivity.this, "noFavorites");
                    }
                }
            })
        ));
        ...
        myItems.add(new TopLevelTreebytitle(
...
/*        myItems.add(new TopLevelTree(
        ...
        myItems.add(new TopLevelTree(
            ...
            new Runnable() {
                public void run() {
                    startActivity(
                        new Intent(LibrarybytitleActivity.this, FileManager.class)
        ...
        TopLevelTreebytitle tree = (TopLevelTreebytitle)((LibraryAdapter)getListAdapter()).getItem(0);
        tree.run();           
    ...
    public void onListItemClick(ListView listView, View view, int position, long rowId) {
        TopLevelTreebytitle tree = (TopLevelTreebytitle)((LibraryAdapter)getListAdapter()).getItem(position);
        tree.run();
    }
    ...
    private void setSearchResults(Intent intent) {
        ...
        mySearchResultsItem = new TopLevelTreebytitle(
...
class TopLevelTreebytitle extends FBTree implements ZLAndroidTree {
    private final ZLResource myResource;
    private final String myParameter;
    private final int myCoverResourceId;
    private final Runnable myAction;

    public TopLevelTreebytitle(ZLResource resource, String parameter, int coverResourceId, Runnable action) {
        myResource = resource;
    ...
    public TopLevelTreebytitle(ZLResource resource, int coverResourceId, Runnable action) {
