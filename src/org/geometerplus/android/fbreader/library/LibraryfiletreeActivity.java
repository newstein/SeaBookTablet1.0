...
public class LibraryfiletreeActivity extends LibraryBaseActivity {
    private LinkedList<FBTree> myItems;
    private TopLevelTreefiletree mySearchResultsItem;
...
/*        myItems.add(new TopLevelTree(
            ...
            new OpenTreeRunnable(LibraryInstance, new StartTreeActivityRunnable(PATH_FAVORITES, null) {
                public void run() {
                    if (LibraryInstance.favorites().hasChildren()) {
                        ...
                        UIUtil.showErrorMessage(LibraryfiletreeActivity.this, "noFavorites");
                    }
                }
            })
        ));
        ...
        myItems.add(new TopLevelTreefiletree(
            myResource.getResource("fileTree"),
            R.drawable.ic_list_library_folder,
            new Runnable() {
                public void run() {
                    startActivity(
                        new Intent(LibraryfiletreeActivity.this, FileManager.class)
        ...
        TopLevelTreefiletree tree = (TopLevelTreefiletree)((LibraryAdapter)getListAdapter()).getItem(0);
        tree.run();            
    ...
    public void onListItemClick(ListView listView, View view, int position, long rowId) {
        TopLevelTreefiletree tree = (TopLevelTreefiletree)((LibraryAdapter)getListAdapter()).getItem(position);
        tree.run();
    }
    ...
    private void setSearchResults(Intent intent) {
        ...
        mySearchResultsItem = new TopLevelTreefiletree(
...
class TopLevelTreefiletree extends FBTree implements ZLAndroidTree {
    private final ZLResource myResource;
    private final String myParameter;
    private final int myCoverResourceId;
    private final Runnable myAction;

    public TopLevelTreefiletree(ZLResource resource, String parameter, int coverResourceId, Runnable action) {
        myResource = resource;
    ...
    public TopLevelTreefiletree(ZLResource resource, int coverResourceId, Runnable action) {
